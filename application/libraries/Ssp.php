...
class SSP {
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        $total_query = "SELECT COUNT(`{$primaryKey}`) FROM `$table`";

        if($extraWhere != '') $total_query = "SELECT COUNT(`{$primaryKey}`) FROM `$table` WHERE ".$originalextraWhere;
        if($joinQuery != '') $total_query = "SELECT COUNT(`{$primaryKey}`) ".$joinQuery;
        if($joinQuery != '' && $extraWhere != '') $total_query = "SELECT COUNT(`{$primaryKey}`) ".$joinQuery." WHERE ".$originalextraWhere;

        $resTotalLength = SSP::sql_exec( $db, $total_query);
