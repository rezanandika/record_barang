...
class Rekapitulasi extends Admin_Controller implements ControllerInterface{
	public function __construct(){
		parent::__construct();
		$this->load->model("Rekapitulasi_model", 'rekap');
		$this->title = 'Data Rekapitulasi';
		$this->redirect_url = base_url_admin().'rekap';
	}

	function index(){
		$data['title']		= $this->title;
		$data['content']	= 'content/rekapitulasi/index';
		$this->rekap->select('transaksi.*');
		$this->rekap->order_by('transaksi.id','DESC');
		$data['datas']		= array();//$this->user->get_all();

		$this->template($data);
	}
	
	function rekap_lunas(){
		$data['title']		= 'Transaksi Lunas';
		$data['content']	= 'content/rekapitulasi/rekap_lunas';
		$this->rekap->select('transaksi.*');
		$this->rekap->order_by('transaksi.id','DESC');
		$data['datas']		= array();//$this->user->get_all();
	...
	function rekap_belum_lunas(){
		$data['title']		= 'Transaksi Belum Lunas';
		$data['content']	= 'content/rekapitulasi/rekap_belum_lunas';
		$this->rekap->select('transaksi.*');
		$this->rekap->order_by('transaksi.id','DESC');
		$data['datas']		= array();//$this->user->get_all();
	...
	function invoice(){
		$data['title']		= 'Invoice';
		$data['content']	= 'content/rekapitulasi/invoice';
		$this->rekap->select('transaksi.*');
		$this->rekap->order_by('transaksi.id','DESC');
		$data['datas']		= array();//$this->user->get_all();
	...
	function form(){
		...
		$this->customer->select('customer.*');
		$this->customer->order_by('customer.id','DESC');
		$data['datas']		= array();//$this->user->get_all();
